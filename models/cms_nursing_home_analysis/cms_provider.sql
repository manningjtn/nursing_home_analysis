...
(
    select
        ...
        ,AVERAGE_NUMBER_OF_RESIDENTS_PER_DAY / NULLIF(NUMBER_OF_CERTIFIED_BEDS, 0) AS AVERAGE_OCCUPANCY
    from {{source('cms_provider', 'PROVIDER_INFO_DEC2025')}}
    where 
        REGISTERED_NURSE_TURNOVER < (select avg(REGISTERED_NURSE_TURNOVER) from {{source('cms_provider', 'PROVIDER_INFO_DEC2025')}})
        and 
        STAFFING_RATING > (SELECT AVG(STAFFING_RATING) FROM {{source('cms_provider', 'PROVIDER_INFO_DEC2025')}})
)
